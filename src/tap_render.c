...
TapResult tap_render_init(TapRenderInitOpts opts)
{
    ...
    if (state.initialized)
        return result;
    if (opts.rad_fovy < 0 || opts.rad_fovy > TAP_MATH_PI*2 || opts.z_near < 0.01F || opts.z_far < opts.z_near || opts.viewport_width == 0 || opts.viewport_height == 0)
...
static void frustum_create(Frustum *frustum, const float rad_fovy, const float ratio, const float z_near, const float z_far, 
                           const TapVec3 eye, const TapVec3 center, const TapVec3 up)
{
...
TapResult tap_render_frustum_check_aabb(TapBool *is_outside, const TapVec3 bbox_min, const TapVec3 bbox_max)
{
    ...
    for (i=0; i < TAP_ARRAY_SIZE(state.frustum.points); ++i)
    {
        TapVec3 pos, neg;
        pos.x = (state.frustum.normals[i].x >= 0) ? bbox_max.x : bbox_min.x;
        pos.y = (state.frustum.normals[i].y >= 0) ? bbox_max.y : bbox_min.y;
        pos.z = (state.frustum.normals[i].z >= 0) ? bbox_max.z : bbox_min.z;

        neg.x = (state.frustum.normals[i].x >= 0) ? bbox_min.x : bbox_max.x;
        neg.y = (state.frustum.normals[i].y >= 0) ? bbox_min.y : bbox_max.y;
        neg.z = (state.frustum.normals[i].z >= 0) ? bbox_min.z : bbox_max.z;
...
TapResult tap_render_frame_setup(TapVec3 eye, TapVec3 center, TapVec3 up)
{
    ...
    frustum_create(&state.frustum, state.metadata.rad_fovy, ratio, state.metadata.z_near, state.metadata.z_far, state.metadata.eye, state.metadata.center, state.metadata.up);
